...
export async function POST(req: Request) {
  try {
    ...
    if (authHeader && authHeader.startsWith('Bearer ')) {
      ...
      user = data.user;
      console.log('🔍 Bearer token auth result:', { user: user?.id, error: error?.message });
    }
    ...
    console.log('Submitting job to dispatcher:', { companyName, address: addr, user_id: user.id });
    ...
    try {
      ...
      if (!dispatcherData || dispatcherData.status !== 'accepted' || !dispatcherData.correlation_id) {
        console.error('Invalid dispatcher response format:', dispatcherData);
        return new Response(JSON.stringify({ 
          success: false, 
          error: "Invalid response from dispatcher. Expected { status: 'accepted', correlation_id: 'uuid' }" 
        }), { status: 500 });
      }
      ...
      if (fetchError.name === 'AbortError') {
        ...
        return new Response(JSON.stringify({ 
          success: false, 
          error: "Request timeout: The enrichment job submission took too long. Please try again." 
        }), { status: 504 });
      }
