...
export function parseScraperWorkerResults(
  ...
  try {
    ...
    if (Array.isArray(scraperWorkerResultsJson?.output)) {
      console.log('✅ Strategy 2: Output array format - searching for assistant message')
    ...
    if (scraperWorkerResultsJson?.output?.[0]?.content?.[0]?.text) {
      console.log('✅ Strategy 3: Legacy Claude API format - parsing from output[0].content[0].text')
    ...
    if (typeof scraperWorkerResultsJson.output === 'string') {
      console.log('✅ Strategy 4: OpenAI string output - parsing from output field')
    ...
    if (scraperWorkerResultsJson.output && typeof scraperWorkerResultsJson.output === 'object' && !Array.isArray(scraperWorkerResultsJson.output)) {
      console.log('✅ Strategy 5: OpenAI object output - using output field directly')
    ...
    console.log('📦 Full object:', JSON.stringify(scraperWorkerResultsJson, null, 2))
    return null
  } catch (error) {
    console.error('❌ Failed to parse scraper_worker_results_json:', error)
    console.log('📦 Problematic data:', JSON.stringify(scraperWorkerResultsJson, null, 2))
    return null
  }
}
 ...
 */
export function scraperOutputToEnrichedJson(scraperOutput: ScraperWorkerOutput): any {
  return {
    ...
    staff: scraperOutput.staff_list?.map(member => ({
      name: member.name,
      role: member.role,
 ...
 */
export function extractScraperMetadata(scraperOutput: ScraperWorkerOutput | null) {
  if (!scraperOutput) {
    return {
